...
export default function RealTimeTickerCharts({
  ...
  }, [realTimeData, isHoveringChart]);

  const transformedRealTimeData = useMemo(
    () => transformDataForLineChart(displayData),
    [displayData]
  );
  ...
  const options: ApexOptions = useMemo(
    () => ({
      ...
      xaxis: {
        type: 'category',
        categories: displayData.map((item) => item.hours),
        labels: {
          formatter: function (val: string) {
            return val?.slice(0, 5) || '';
          },
        },
        tickAmount: Math.min(5, displayData.length),
      },
      tooltip: {
        enabled: true,
        shared: false,
  ...
  return (
    ...
    >
      <ApexChart options={options} series={series} type="area" height={380} />
    </Wrapper>
  );
}
...
const Wrapper = styled.div`
  ...
  .apexcharts-tooltip {
    transform: translate(calc(-100% - 16px), -8px) !important;
  }
